...
struct boid_fig
{
    ...
    boid_fig()
    {
        ...
        vector<int> boid_i = { 0 , 3 , 1 , 0 , 3 , 2 };

        glGenBuffers(1,&x_vbo);
        glBindBuffer(GL_ARRAY_BUFFER,x_vbo);
        glEnableVertexAttribArray(0);
        glBufferData(GL_ARRAY_BUFFER,sizeof(glm::vec3)*boid_x.size(),&boid_x[0],GL_STATIC_DRAW);
        glVertexAttribPointer(0,3,GL_FLOAT,GL_FALSE,0,0);

        glGenBuffers(1,&t_vbo);
        glBindBuffer(GL_ARRAY_BUFFER,t_vbo);
        glEnableVertexAttribArray(1);
        glBufferData(GL_ARRAY_BUFFER,sizeof(glm::vec2)*boid_t.size(),&boid_t[0],GL_STATIC_DRAW);
        glVertexAttribPointer(1,2,GL_FLOAT,GL_FALSE,0,0);

        glGenBuffers(1,&i_vbo);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,i_vbo);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER,sizeof(int)*boid_i.size(),&boid_i[0],GL_STATIC_DRAW);
